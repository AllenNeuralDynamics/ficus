...
def test_update_rigname_and_hostname(client, api_prefix):
    response = client.patch(f"{api_prefix}/rigs/foo_1_a", json={
        "rig_name": "foonew_3_c", 
    ...
    response = client.get(f"{api_prefix}/rigs/foonew_3_c")
    assert response.status_code == 200
...
def test_update_rigname_only(client, api_prefix):
    response = client.patch(f"{api_prefix}/rigs/foo_1_a", json={
        "rig_name": "foonew_3_c", 
    })
    assert response.status_code == 200
...
def test_update_hostname_only(client, api_prefix):
    response = client.patch(f"{api_prefix}/rigs/foo_1_a", json={
        "hostname": "w11test-foonew"
    })
    assert response.status_code == 200
...
def test_update_non_existing_rig(client, api_prefix):
    response = client.patch(f"{api_prefix}/rigs/badrig_3_bad", json={
        "rig_name": "foonew_3_c", 
    })
...
def test_update_invalid_new_rigname(client, api_prefix):
    response = client.patch(f"{api_prefix}/rigs/foo_1_a", json={
...
def test_update_rig_already_exists(client, api_prefix): 
    response = client.patch(f"{api_prefix}/rigs/foo_1_a", json={
        "rig_name": "bar_2_b", 
    })
    assert response.status_code == 404
